...
class RNA_imem(InMemoryDataset):
    ...
    allowable_features = {
        ...
        'possible_nucleotide_acids': ['A','U','G','C'],
    ...
    def __init__(self, root: str, name: str,
        ...
        self.c_4_max_neighbors = c_4_max_neighbors
        ...
        self.wrong_rnas = ['1kp0A01', '2atcA02']
    ...
    def write_info(self):
        written_filename = os.path.join(self.root, 'wrong_rna_names.txt')
        file = open(written_filename, "w+")
        for rna_name in self.wrong_rna:
            file.writelines(rna_name + '\n')
        file.close()

    def generate_rna_graph(self):
        ...
        for idx, name in enumerate(tqdm(names)):
            ...
            if os.path.exists(saved_graph_filename):
                continue
            rna_filename = os.path.join(self.raw_file_names, name)
            if (name in self.wrong_rnas) or (not rna_filename):
                continue
            rec, rec_coords, c_4_coords, p_coords, n_coords = self.get_receptor_inference(rna_filename)
    ...
    def rec_residue_featurizer(self, rec, one_hot=True, add_feature=None):
        ...
        try:
            ...
            for residue in rec.get_residues():
                ...
                residx = safe_index(
                    self.allowable_features['possible_nucleotide_acids'], residue.get_resname())
                # print(residx)
    ...
    def get_node_features(self, p_coords, n_coords, c_4_coords, coord_mask, with_coord_mask=True, use_angle=False, use_omega=False):
        num_res = p_coords.shape[0]
        if use_omega:
            num_angle_type = 3
            angles = np.zeros((num_res, num_angle_type))
            for i in range(num_res-1):
                # These angles are called ¦Õ (phi) which involves the backbone atoms C-N-C¦Á-C
                angles[i, 0] = dihedral(
                    n_coords[i], p_coords[i], c_4_coords[i], p_coords[i+1])

                # psi involves the backbone atoms N-C¦Á-C-N.
                angles[i, 1] = dihedral(
                    p_coords[i], c_4_coords[i], n_coords[i], p_coords[i+1])
                angles[i, 2] = dihedral(
                    c_4_coords[i], n_coords[i], p_coords[i+1], c_4_coords[i+1])
        else:
            num_angle_type = 2
            angles = np.zeros((num_res, num_angle_type))
            for i in range(num_res-2):
                # RNA phi 2 angle 
                angles[i, 0] = dihedral(
                    c_4_coords[i], p_coords[i+1], c_4_coords[i+1], p_coords[i+2])
            for i in range(num_res-1):
                # RNA phi 4 angle
                angles[i, 1] = dihedral(
                    p_coords[i], c_4_coords[i], p_coords[i+1], c_4_coords[i+1])
        if use_angle:
    ...
    def get_c4prime_graph(self, rec, c_4_coords, p_coords, n_coords, coords, struc_2nd_res = None):

        scalar_feature, vec_feature = self.get_node_features(
            p_coords, n_coords, c_4_coords, coord_mask=None, with_coord_mask=False, use_angle=self.use_angle, use_omega=self.use_omega)
        ...
        for i, residue in enumerate(rec.get_residues()):
            num_res = len([_ for _ in rec.get_residues()])
            p_coord = p_coords[i]
            c_4_coord = c_4_coords[i]
            n_coord = n_coords[i]
            u_i = (p_coord - c_4_coord) / \
                np.linalg.norm(p_coord - c_4_coord)
            t_i = (n_coord - c_4_coord) / \
                np.linalg.norm(n_coord - c_4_coord)
            n_i = np.cross(u_i, t_i) / \
                np.linalg.norm(np.cross(u_i, t_i))   # main chain
            v_i = np.cross(n_i, u_i)
            assert (math.fabs(
                np.linalg.norm(v_i) - 1.) < 1e-5), "rna utils rna_to_graph_dips, v_i norm larger than 1"
            n_i_list.append(n_i)
            u_i_list.append(u_i)
            v_i_list.append(v_i)
            residue_representatives_loc_list.append(c_4_coord)
        ...
        distances = spa.distance.cdist(c_4_coords, c_4_coords)
        ...
        for i in range(num_residues):
            ...
            dst.remove(i)
            if self.c_4_max_neighbors != None and len(dst) > self.c_4_max_neighbors:
                dst = list(np.argsort(distances[i, :]))[
                    1: self.c_4_max_neighbors + 1]
            if len(dst) == 0:
                # choose second because first is i itself
                dst = list(np.argsort(distances[i, :]))[1:2]
                log(
                    f'The c_4_cutoff {self.cutoff} was too small for one c_4 such that it had no neighbors. So we connected it to the closest other c_4')
            assert i not in dst
            src = [i] * len(dst)
            src_list.extend(src)
            dst_list.extend(dst)
    ...
    def get_receptor_inference(self, rec_path):
        ...
        for i, chain in enumerate(rec):
            ...
            for res_idx, residue in enumerate(chain):
                ...
                num_res = len([_ for _ in rec.get_residues()])
                residue_coords = []
                c_4, p, n = None, None, None
                for atom in residue:
                    if atom.name == 'C4\'':
                        c_4 = list(atom.get_vector())
                    if atom.name == 'P':
                        p = list(atom.get_vector())
                ...
                if c_4 != None and p != None and n != None:
                    chain_c_4_coords.append(c_4)
                    chain_p_coords.append(p)
            ...
            c_4_coords.append(np.array(chain_c_4_coords))
        ...
        for i, chain in enumerate(rec):
            if chain.get_id() in valid_chain_ids:
                ...
                valid_c_4_coords.append(c_4_coords[i])
        ...
        c_4_coords = np.concatenate(valid_c_4_coords, axis=0)  # [n_residues, 3]
        p_coords = np.concatenate(valid_p_coords, axis=0)  # [n_residues, 3]
        n_coords = np.concatenate(valid_n_coords, axis=0)  # [n_residues, 3]

        for invalid_id in invalid_chain_ids:
            rec.detach_child(invalid_id)
        num_res = len([_ for _ in rec.get_residues()])
        assert len(c_4_coords) == len(p_coords)
        assert len(c_4_coords) == len(n_coords)
        assert sum(valid_lengths) == len(c_4_coords)
        return rec, coords, c_4_coords, p_coords, n_coords
    ...
    def get(self, idx):
        if self.random_sampling:
            ...
            idx_rna = torch.where(idx < self.nums_amino_cum)[0][0] - 1
        idx_x0 = self.slices['x'][idx_rna]
        idx_x1 = self.slices['x'][idx_rna+1]
        idx_edge0 = self.slices['edge_index'][idx_rna]
        idx_edge1 = self.slices['edge_index'][idx_rna+1]
        ...
        if self.diffusion:
            ...
            if self.random_sampling:
                ...
                subidx = int(idx - self.nums_amino_cum[idx_rna])
    ...
    def find_idx(self, idx_rna, amino_idx):
        idx = (self.distances[idx_rna][:-1, amino_idx]
               < self.micro_radius).nonzero(as_tuple=True)[0]
        return idx

    def get_c4prime_graph_single(self, graph, idx_rna, amino_idx):
        choosen_amino_idx = self.find_idx(idx_rna, amino_idx)
        keep_edge_index = []
