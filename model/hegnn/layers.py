...
class EGNN_Sparse(MessagePassing):
        ...
        **kwargs
        ...
        if self.mlp_num >2:
            self.node_mlp = nn.Sequential(
                HypLinear(config['manifold'], 1, hidden_channels, config['c'], dropout, use_bias=True), 
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        #print("!!!!!",self.ball.sqdist(coors[edge_index[0]], coors[edge_index[1]],1).shape)
        ...
        if self.ball_sqdist:
            rel_coors = self.ball.sqdist(coors[edge_index[0]], coors[edge_index[1]],1).unsqueeze(-1)
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        try:
            ...
            coll_dict = self.__collect__(self._user_args,
        ...
        if self.update_coors:
            ...
            if self.use_bias:
                bias = self.manifold.proj_tan0(mhat_i, self.c)
                hyp_bias = self.manifold.expmap0(bias, self.c)
                hyp_bias = self.manifold.proj(hyp_bias, self.c)
                res = self.manifold.mobius_add(res, kwargs["coors"], c=self.c)
                coors_out = self.manifold.proj(res, self.c)
            else:
        ...
        if self.update_feats:
            ...
            if self.use_bias:
                bias = self.manifold.proj_tan0(hidden_out, self.c)
                hyp_bias = self.manifold.expmap0(bias, self.c)
                hyp_bias = self.manifold.proj(hyp_bias, self.c)
                res = self.manifold.mobius_add(res, kwargs["x"], c=self.c)
